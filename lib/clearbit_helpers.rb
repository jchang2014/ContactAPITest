module ClearbitHelpers
	def create_cb_profile(user)
		...
		@cb_profile.linkedin_url = find_cb_linkedin_url(@clearbit_response)
		@cb_profile.save
	end

	def find_cb_linkedin_url(response)
		@url = response.try(:person).try(:linkedin).try(:handle)
